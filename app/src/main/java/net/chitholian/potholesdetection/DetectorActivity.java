...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  private MediaPlayer mpSlow;
  private MediaPlayer mpMed;
  private MediaPlayer mpFast;
              ...
              public void run() {
                ...
                for (final Classifier.Recognition result : results) {
                  ...
                  if (location != null && result.getConfidence() >= MINIMUM_CONFIDENCE_TF_OD_API) {
                    float potholeSize = location.width() * location.height();
                    canvas.drawRect(location, paint);
                    LOGGER.i("Bounding Box Area: " + potholeSize);
                    int numPotholes = tracker.trackedObjects.size();
                    if (numPotholes == 0)
                    {
                      stopAllAlarms();
                    }
                    else {
                      if (potholeSize > 13000) {
                        mpSlow.pause();
                        mpSlow.seekTo(0);
                        mpMed.pause();
                        mpMed.seekTo(0);
                        playAlarm(mpFast);
                      } else if (potholeSize > 7000 && potholeSize < 12000) {
                        mpSlow.pause();
                        mpSlow.seekTo(0);
                        playAlarm(mpMed);
                      } else {
...
//                  stopAllAlarms();
//                }
//                else if (numPotholes == 1) {
//                  playAlarm(mpSlow);
//                } else if (numPotholes >= 2 && numPotholes <= 3) {
//                  mpSlow.pause();
//                  mpSlow.seekTo(0);
//                  playAlarm(mpMed);
//                } else {
...
//                }
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mpSlow = MediaPlayer.create(this, R.raw.alarm_slow);
    mpMed = MediaPlayer.create(this, R.raw.alarm_med);
    mpFast = MediaPlayer.create(this, R.raw.alarm_fast);
  }
